# Leituras

## Aula 12 - Introdução ao Deep Learning+PyTorch
___

**Leituras Obrigatórias:**  
- Gareth James, Gareth; Witten, Daniela; Hastie, Trevor; Tibshirani, Robert. (2023). {cite}`james2023introduction`.*An Introduction to Statistical Learning: with Applications in Python*. Springer. Capítulo 10 - Deep Learning.  


**Objetivos da aula:**  
- Introduzir os alunos aos principais conceitos de Deep Learning;
- Introduzir os alunos o Pytorch, nossa ferramenta de Deep Learning para o Python.


**Leituras Complementares:**

- Géron, Aurelien. (2022). {cite}`geron2022hands`."10. Introduction to Artificial Neural Networks with Keras” and “11. Training Deep Neural Networks” in *Hands-On Machine Learning with Scikit-Learn, Keras, and TensorFlow: Concepts, Tools, and Techniques to Build Intelligent Systems*.  
- Eli Stevens, Eli; Antiga, Luca; Viehmann, Thomas. (2020). {cite}`stevens2020deep`.Deep Learning with PyTorch: Build, Train, and Tune Neural Networks Using Python Tools. Manning Publications.



**Laboratório 12: Introdução ao Deep Learning**  






