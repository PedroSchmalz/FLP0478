...
[^1]: Grimmer et al. (2022) argumentam que não existe um valor "correto", e que os rótulos devem ser validados pelo contexto da anotação e conhecimento da literatura e da teoria. "Por agnóstico, queremos dizer que, em muitas instâncias, não existem valores 'verdadeiros' para que possamos direcionar os métodos de texto como dado, ou nenhum modelo único que possa ser usado para todas as aplicações de um corpus." ({cite}`grimmer2022text`, p.49, tradução nossa).
